...
const CommProjetRetenus = () => {
  ...
  const fetchProjectTitles = async (start, end) => {
    try {
      const response = await DetailsProjets.getProjectTitlesByDateAndState(start, end, 'RETENU');
 ...
 <>
    ...
    <h2 className="text-3xl font-semibold leading-tight text-black  sm:text-2xl xl:text-4xl mb-6">
       Visualisation des projets Retenus
      </h2>
    ...
    <div className="bg-white  shadow-md ring-gray-900/5 flex flex-wrap justify-center mt-10">
        ...
        <span
            className="grid h-20 w-20 place-items-center rounded-full bg-primary transition-all duration-300 group-hover:bg-secondary-300">
                  <img src={projetRetenue} alt="projetRetenue" className="h-10 w-10 text-white transition-all" />
                </span>
                <div
                ...
                <div className="pt-5 text-base font-semibold leading-7">
                  <p>
                  <Link to={`/Commdetails-retenus/${startDate}/${endDate}/${title}/${id}`} className="text-black transition-all duration-300 group-hover:text-white">Ouvrir &rarr;</Link>
                  </p>
                </div>
              </div>
            </div>
...
export default CommProjetRetenus;