...
const Form = ({
  ...
  const { projectId } = useParams(); // Récupérer l'ID du projet de l'URL
  ...
  useEffect(() => {
    if (projectId) {
      CreateProject.getProjectDetails(projectId)
        .then((response) => {
          // Mettez à jour l'état avec les détails du projet
  ...
  const [expired,setExpired] = useState(false);
   ...
   // Mettez à jour les valeurs de chaque champ du formulaire avec les détails du projet
   useEffect(() => {
    setEmail(projectDetails.email || '');
    setIntituleProjet(projectDetails.intituleProjet || '');
    setIdeeEntreprise(projectDetails.ideeEntreprise || '');
    setDeveloppezVotreIdee(projectDetails.developpezVotreIdee || '');
    setDiplome(projectDetails.diplome || '');
    setDomaineProjetEntreprise(projectDetails.domaineProjetEntreprise || '');
    setEtablissement(projectDetails.etablissement || '');
    setFiliere(projectDetails.filiere || '');
    setImpactEconomique(projectDetails.impactEconomique || '');
    setInnovationTechnologique(projectDetails.innovationTechnologique || '');
    setMembers(projectDetails.members || '');
    setMotivations(projectDetails.motivations || '');
    setNomPrenom(projectDetails.nomPrenom || '');
    setOriginalite(projectDetails.originalite || '');
    setPerennitePotentielDeveloppement(projectDetails.perennitePotentielDeveloppement || '');
    setResponsabilitesSocialesEnvironnementales(projectDetails.responsabilitesSocialesEnvironnementales || '');
    setScanCarteEtudiantFile(projectDetails.scanCarteEtudiant || '');
    setTelephonePortable(projectDetails.telephonePortable || '');
    setViabiliteDurableteProjet(projectDetails.viabiliteDurableteProjet || '');
    setVilleOrigine(projectDetails.villeOrigine || '');
  }, [projectDetails]);
  useEffect(() => {
  ...
  const saveProject = (e) => {
    ...
    CreateProject.checkExistingProject2(intituleProjet,projectId)
      .then((response) => {
    ...
    scanCarteEtudiantImages.forEach((fileURL, index) => {
      localStorage.setItem(`${uploadDir}${scanCarteEtudiantFile[index].name}`, fileURL);
          ...
          // Mettre à jour le projet avec l'ID spécifié
          CreateProject.updateProject(projectId, formData)
            .then((response) => {
              Modal.success({
                title: 'Succès',
                content: 'Projet mis à jour avec succès !',
            ...
            .catch((error) => {
              Modal.error({
                title: 'Erreur',
                content: `Erreur lors de la mise à jour du projet : ${error.message}`,
              });
            });
        }
      })
